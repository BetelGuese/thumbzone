/**
 * Pointer-driven drag-to-dismiss and swipe-to-open, shared by every system.
 *
 * Split out of `core/behaviour.js` the way each port used to split its own
 * copy out of its lifecycle module: that module owns open, close, focus,
 * `inert`, the menu order and teardown, and stays readable only if the gesture
 * engine lives beside it. This has no purpose or lifecycle of its own outside
 * `createThumbzoneBehaviour`, its only caller.
 *
 * Every tuned value and every piece of arithmetic comes from `core/index.js`,
 * which is why nothing here compares a distance against a threshold or divides
 * a delta by a duration of its own.
 */

import { SWIPE_OPEN_DISTANCE, createVelocityTracker, dragProgress, shouldDismiss } from './index.js'

/**
 * Wires the gestures over an already-open-capable sheet and trigger.
 *
 * The inline `transform` is deliberately not restored by `detach()`: the
 * sheet's pre-init inline value is the lifecycle's to capture and hand back,
 * and it does so straight after calling this.
 *
 * @param {object} deps
 * @param {HTMLElement} deps.sheet
 * @param {HTMLElement} deps.trigger
 * @param {HTMLElement} deps.menu The scroll container a drag may never start inside.
 * @param {() => void} deps.open
 * @param {() => void} deps.close
 * @returns {{ detach: () => void, consumeSwipeClick: () => boolean }}
 */
export function attachGestures({ sheet, trigger, menu, open, close }) {
  /**
   * The gesture in flight, if any.
   *
   * `source` is what the pointer went down on, and it is what every later
   * event is matched against: a drag on the sheet and a swipe on the trigger
   * are different gestures with different outcomes, and only one can be live
   * at a time. `capturedBy` is the element holding the pointer capture, which
   * is how an abandoned drag is recognised — see `clearStaleDrag`.
   * @type {{ source: 'sheet' | 'trigger', pointerId: number, startY: number,
   *   tracker: ReturnType<typeof createVelocityTracker>, capturedBy: HTMLElement } | null}
   */
  let drag = null
  ...
  // currently active pointer behind it (verified on both engines) — rare, but
  // reachable through a stray or malformed event. Committing to a drag only
  // once capture has genuinely succeeded keeps `drag` from being set while no
  // capture is held, which every guard below would then honour until
  // clearStaleDrag() happened to notice.
  function beginDrag(target, event, source) {
    try {
  ...
  function resetDragVisuals() {
    ...
    // that puts the authored value back instead, and the lifecycle does that.
  ...
  // state to recover from — treat it as gone before deciding whether a new
  // pointerdown may proceed.
  ...
  // Chromium; once a press on one moves far enough, Chromium commits to its
  // own drag-and-drop gesture and cancels our pointer stream — a
  // 'pointercancel' with the coordinates zeroed — instead of ever delivering a
  // pointerup, so the release is never evaluated at all. WebKit does not do
  // this. Preventing 'dragstart' (it bubbles up from the link or image) stops
  // that commitment on the sheet and the trigger alike, whatever content a
  // consumer puts in either.
  ...
  function onSheetPointerDown(event) {
    ...
    // The menu is the sheet's own scroll container and stays pannable by touch
    // at every scroll position, so a drag never starts inside it regardless of
    // where it happens to be scrolled — which leaves the handle, authored as
    // the menu's sibling above it, as the one surface a dismiss drag can begin
    // on. That is what the menu's static touch-action depends on holding.
  ...
  function onSheetPointerMove(event) {
    ...
    // the interface imposes on its own, not motion the user is actively
    // driving with a finger, and freezing direct manipulation would read as
    // broken rather than calmer. The release below clears the override and
    // hands the settle back to the stylesheet, whose transitions the
    // preference does collapse.
  ...
  // dismiss a gesture the browser aborted rather than one the user released.
  // So this resets the same state a release does and always springs back, on
  // the trigger as well as the sheet: a swipe cancelled by the platform must
  // not leave `drag` permanently set either.
  ...
  return {
    detach() {
      ...
      // A drag mid-flight at teardown holds a real pointer capture, and
      // dropping the reference to it does not end it: until that pointer is
      // lifted, the element keeps receiving events that now belong to nobody.
      // Handing the capture back makes the teardown total rather than merely
      // quiet — and it is guarded, because releasing a capture the element
      // does not hold throws.
      ...
      // transition disabled, so every future open and close would be dead
      // until the next drag happened to clear it.
      delete sheet.dataset.tzDragging
    },
    // Check-and-clear in one step: a second click shortly after a swipe (e.g.
    // a genuine follow-up tap) must be treated as the ordinary tap it is, not
    // silently eaten too.
    consumeSwipeClick() {
